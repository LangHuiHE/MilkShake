...
type Users struct {
	AppAuthToken     string    `orm:"column(appAuthToken);size(128);null"`
	ContactAddress   string    `orm:"column(contactAddress);size(50);null"`
	EmailAddress     string    `orm:"column(emailAddress);size(30);null"`
	EnrollDate       time.Time `orm:"column(enrollDate);type(date);null"`
	FirstName        string    `orm:"column(firstName);size(20)"`
	LastName         string    `orm:"column(lastName);size(20)"`
	PermanentAddress string    `orm:"column(permanentAddress);size(50);null"`
	PhoneNumber      string    `orm:"column(phoneNumber);size(15);null"`
	Ssn              string    `orm:"column(ssn);size(11);null"`
	StatusType       int       `orm:"column(statusType);null"`
	Id               int       `orm:"column(userID);auto"`
	UserPassword     string    `orm:"column(userPassword);size(128)"`
